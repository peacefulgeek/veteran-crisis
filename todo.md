...
- [ ] (Operator-only) Lower Bunny Pull Zone "Cache Expiration Time" for `articles/*.json` from 30d to ~5min in the Bunny dashboard, OR run `BUNNY_ACCOUNT_API_KEY=... node scripts/purge-bunny-edge-cache.mjs`. Either action drops the residual ~30-day stale-edge window. Cannot be done from sandbox without the Account API Key (the storage Access Key returns 401 against api.bunny.net/purge — verified).
